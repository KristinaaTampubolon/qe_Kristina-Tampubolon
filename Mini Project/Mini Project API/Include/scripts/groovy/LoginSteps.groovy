...
class LoginSteps {
	...
	def postValidLogin() {
		statusCode = WS.sendRequest(findTestObject('Postman (1)/Authentication/Login', [('BASE_URL') : GlobalVariable.BASE_URL]))
	}
	...
	def postLoginEmpty() {
		statusCode = WS.sendRequest(findTestObject('Postman (1)/Authentication/Login_empty data'))
	}
	...
	def responseLoginEmptyEmail() {
		statusCode = WS.sendRequest(findTestObject('Postman (1)/Authentication/Login_empty email'))
	}
	@Given("Post call to url login with empty password")
	def responseLoginEmptyPassword() {
		statusCode = WS.sendRequest(findTestObject('Postman (1)/Authentication/Login_empty password'))
	}
